...
func (t *TradeWorkflowChaincode) requestTrade(stub shim.ChaincodeStubInterface, creatorOrg string, creatorCertIssuer string, args []string) pb.Response {
	...
	if t.testMode == true && is_role_attribute(stub, "importer") == false {
		return shim.Error("Caller not a member of Importer Org. Access denied.")
	}
...
func (t *TradeWorkflowChaincode) acceptTrade(stub shim.ChaincodeStubInterface, creatorOrg string, creatorCertIssuer string, args []string) pb.Response {
	...
	if t.testMode == true && is_role_attribute(stub, "exporter") == false {
		return shim.Error("Caller not a member of Exporter Org. Access denied.")
	}
...
func (t *TradeWorkflowChaincode) requestAcceptedLC(stub shim.ChaincodeStubInterface, creatorOrg string, creatorCertIssuer string, args []string) pb.Response {
	...
	var tradeAgreementBytes, letterOfCreditBytes, exporterBytes []byte
	var tradeAgreement *TradeAgreement
	var acceptedLetterOfCredit *AcceptedLetterOfCredit
	var err error
	...
	if t.testMode == true && is_role_attribute(stub, "lender") == false {
		return shim.Error("Caller not a member of Lender Org. Access denied.")
	}
	...
	acceptedLetterOfCredit = &AcceptedLetterOfCredit{"", "", string(exporterBytes), tradeAgreement.Amount, []string{}, REQUESTED}
	letterOfCreditBytes, err = json.Marshal(acceptedLetterOfCredit)
	if err != nil {
		return shim.Error("Error marshaling letter of credit structure")
	}
...
func (t *TradeWorkflowChaincode) issueAcceptedLC(stub shim.ChaincodeStubInterface, creatorOrg string, creatorCertIssuer string, args []string) pb.Response {
	var lcKey string
	var acceptedLetterOfCreditBytes []byte
	var acceptedLetterOfCredit *AcceptedLetterOfCredit
	var err error
	...
	if len(args) < 3 {
		err = errors.New(fmt.Sprintf("Incorrect number of arguments. Expecting at least 3: {Trade ID, Accepted L/C ID, Expiry Date} [List of Documents]. Found %d", len(args)))
		return shim.Error(err.Error())
	}
	...
	acceptedLetterOfCreditBytes, err = stub.GetState(lcKey)
	if err != nil {
		return shim.Error(err.Error())
	}

	// Unmarshal the JSON
	err = json.Unmarshal(acceptedLetterOfCreditBytes, &acceptedLetterOfCredit)
	if err != nil {
		return shim.Error(err.Error())
	}

	if acceptedLetterOfCredit.Status == ISSUED {
		fmt.Printf("L/C for trade %s already issued", args[0])
	} else if acceptedLetterOfCredit.Status == ACCEPTED {
		fmt.Printf("L/C for trade %s already accepted", args[0])
	} else {
		acceptedLetterOfCredit.Id = args[1]
		acceptedLetterOfCredit.ExpirationDate = args[2]
		acceptedLetterOfCredit.Documents = args[3:]
		acceptedLetterOfCredit.Status = ISSUED
		acceptedLetterOfCreditBytes, err = json.Marshal(acceptedLetterOfCredit)
		if err != nil {
			return shim.Error("Error marshaling L/C structure")
		}
		// Write the state to the ledger
		err = stub.PutState(lcKey, acceptedLetterOfCreditBytes)
		if err != nil {
			return shim.Error(err.Error())
		}
	}
...
func (t *TradeWorkflowChaincode) acceptAcceptedLC(stub shim.ChaincodeStubInterface, creatorOrg string, creatorCertIssuer string, args []string) pb.Response {
	...
	if acceptedLetterOfCredit.Status == ACCEPTED {
		fmt.Printf("L/C for trade %s already accepted", args[0])
	} else if acceptedLetterOfCredit.Status == REQUESTED {
		fmt.Printf("L/C for trade %s has not been issued", args[0])
		return shim.Error("L/C not issued yet")
	} else {
		acceptedLetterOfCredit.Status = ACCEPTED
		acceptedLetterOfCreditBytes, err = json.Marshal(acceptedLetterOfCredit)
		if err != nil {
			return shim.Error("Error marshaling L/C structure")
		}
...
func (t *TradeWorkflowChaincode) issueEL(stub shim.ChaincodeStubInterface, creatorOrg string, creatorCertIssuer string, args []string) pb.Response {
	...
	if t.testMode == true && is_role_attribute(stub, "regulator") == false {
		return shim.Error("Caller not a member of Regulator Org. Access denied.")
	}
...
func (t *TradeWorkflowChaincode) acceptShipmentAndIssueBL(stub shim.ChaincodeStubInterface, creatorOrg string, creatorCertIssuer string, args []string) pb.Response {
	...
	if t.testMode == true && is_role_attribute(stub, "carrier") == false {
		return shim.Error("Caller not a member of Carrier Org. Access denied.")
	}
...
func (t *TradeWorkflowChaincode) requestPayment(stub shim.ChaincodeStubInterface, creatorOrg string, creatorCertIssuer string, args []string) pb.Response {
	...
	var attribute = "role"
	role, found, err = getCustomAttribute(stub, attribute)
	if t.testMode && found && (role != "exporter" && role != "lender") {
		return shim.Error("Caller not a member of Exporter or Lender Org. Access denied.")
	}
	...
	if found && role == "lender" {
		tradeAgreement.Payment = 0 // reset the payment to 0. Lender takes on the trade agreement now
	}
...
func (t *TradeWorkflowChaincode) makePayment(stub shim.ChaincodeStubInterface, creatorOrg string, creatorCertIssuer string, args []string) pb.Response {
	...
	var shipmentLocationBytes, paymentBytes, tradeAgreementBytes, impBalBytes, expBalBytes, lenBalBytes []byte
	var tradeAgreement *TradeAgreement
	var err error
	var role string
	var found bool
	var paymentDiscount float32

	// Access control: Only an Importer Org member can invoke this transaction
	// ME: LENDER ---------------------------------------------------------------------
	if !t.testMode && (!authenticateImporterOrg(creatorOrg, creatorCertIssuer) || !authenticateLenderOrg(creatorOrg, creatorCertIssuer)) {
		return shim.Error("Caller not a member of Importer Org. Access denied.")
	}
	var attribute = "role"
	role, found, err = getCustomAttribute(stub, attribute)
	if t.testMode && found && (role != "importer" && role != "lender") {
		return shim.Error("Caller not a member of Importer or Lender Org. Access denied.")
	}

	if len(args) != 2 {
		err = errors.New(fmt.Sprintf("Incorrect number of arguments. Expecting 2: {Trade ID} {recipient}. Found %d", len(args)))
		return shim.Error(err.Error())
	}
	...
	// account for discount rate
	paymentAmount = int((1 - paymentDiscount) * float32(paymentAmount))

	tradeAgreement.Payment += paymentAmount
	var recipient = args[1]

	// case switch for role and recipient of payment
	if role == "importer" {
		if recipient == "exporter" {
			expBal += paymentAmount
		} else if recipient == "lender" {
		...
		if recipient == "exporter" {
			...
			if lenBal < paymentAmount {
				fmt.Printf("Lender balance %d is insufficient to cover payment amount %d\n", lenBal, paymentAmount)
			}
			lenBal -= paymentAmount
		}
	}
...
func (t *TradeWorkflowChaincode) getELStatus(stub shim.ChaincodeStubInterface, creatorOrg string, creatorCertIssuer string, args []string) pb.Response {
	...
	//}
	// ME: LENDER -----------------------------------------
	if !t.testMode && !(authenticateLenderOrg(creatorOrg, creatorCertIssuer) || authenticateRegulatorOrg(creatorOrg, creatorCertIssuer)) {
		return shim.Error("Caller not a member of Lender or Regulator Org. Access denied.")
	}
...
func (t *TradeWorkflowChaincode) getShipmentLocation(stub shim.ChaincodeStubInterface, creatorOrg string, creatorCertIssuer string, args []string) pb.Response {
	...
	if !t.testMode && !(authenticateImporterOrg(creatorOrg, creatorCertIssuer) || authenticateExporterOrg(creatorOrg, creatorCertIssuer) || authenticateLenderOrg(creatorOrg, creatorCertIssuer) || authenticateCarrierOrg(creatorOrg, creatorCertIssuer)) {
		return shim.Error("Caller not a member of Importer or Exporter or Lender or Carrier Org. Access denied.")
	}
...
// get role attribute specified in string arg in .../network/devmode/createIdentity.sh
// only in test mode
func is_role_attribute(stub shim.ChaincodeStubInterface, input_role string) bool {
	var attribute = "role"
	var role string
	var found bool
	var err error
	...
	if found {
		...
		fmt.Printf("role is not found in input.")
