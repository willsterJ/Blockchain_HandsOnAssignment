...
function upgradeNetwork () {
  ...
  # ME: LENDER --------------------------------------------------------------------------------
  for PEER in peer0.lenderorg.trade.com peer0.importerorg.trade.com peer0.carrierorg.trade.com peer0.regulatororg.trade.com; do
...
function networkClean () {
  ...
  # ME: LENDER --------------------------------------------------------------
  rm -rf channel-artifacts crypto-config add_lender/crypto-config
  # remove the docker-compose yaml file that was customized to the example
  #rm -f docker-compose-e2e.yaml add_org/docker-compose-exportingEntityOrg.yaml
  # --
  rm -f docker-compose-e2e.yaml add_org/docker-compose-exportingEntityOrg.yaml
  rm -f docker-compose-e2e.yaml add_lender/docker-compose-lenderOrg.yaml
  # remove client certs 
  rm -rf client-certs
  if [ "$DEV_MODE" = true ] ; then
     popd
...
function replacePrivateKeyForNewOrg () {
  ...
  # ME: LENDER --------------------------------------------------------------------
  cp add_lender/docker-compose-lenderOrg-template.yaml add_lender/docker-compose-lenderOrg.yaml
  ...
  if [ $(uname -s) == 'Darwin' ] ; then
    ...
    # ME: LENDER ----------------------------------------------------------------------
    sed -i '' "s/LENDER_CA_PRIVATE_KEY/${PRIV_KEY}/g" add_lender/docker-compose-lenderOrg.yaml
  else
    #sed -i "s/EXPORTINGENTITY_CA_PRIVATE_KEY/${PRIV_KEY}/g" add_org/docker-compose-exportingEntityOrg.yaml
    # ME: LENDER --------------------------------------------------------------------
    sed -i "s/LENDER_CA_PRIVATE_KEY/${PRIV_KEY}/g" add_lender/docker-compose-lenderOrg.yaml
...
function generateCertsForNewOrg (){
  ...
  # ME: LENDER --------------------------------------------------------
  cryptogen generate --config=./add_lender/crypto-config.yaml
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate certificates..."
...
function generateChannelConfigForNewOrg() {
  ...
  # ME: LENDER -----------------------------------------------------------------------
  FABRIC_CFG_PATH=${PWD}/add_lender/ && configtxgen -printOrg LenderOrgMSP -channelID $CHANNEL_NAME > ./channel-artifacts/lenderOrg.json
  res=$?
  set +x
  if [ $res -ne 0 ]; then
...
# ME: LENDER ----------------------------------------------------------------
COMPOSE_FILE_NEW_ORG=add_lender/docker-compose-lenderOrg.yaml
# default image tag
IMAGETAG="latest"
# default log file
